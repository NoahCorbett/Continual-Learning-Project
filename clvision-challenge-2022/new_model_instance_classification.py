...
import pytorchcv.models.regnet
import torch
...
from avalanche.training.plugins import lr_scheduling
from torch.optim import lr_scheduler

from avalanche.training.plugins import EvaluationPlugin

from avalanche.training.plugins.mas import MASPlugin
...
DATASET_PATH = Path.home() / '3rd_clvision_challenge' / 'demo'
TRAINING_JSON = 'ego_objects_demo_train.json'
TESTING_JSON = 'ego_objects_demo_test.json'
...
def main(args):
    ...
    benchmark = challenge_classification_benchmark(
        ...
        train_json_name=TRAINING_JSON,
        test_json_name=TESTING_JSON
    ...
    model = SimpleMLP(
        ...
        """
    model = pytorchcv.models.regnet.regnetx016(pretrained=True)
    # ---------
    ...
    optimizer = SGD(model.parameters(), lr=0.02, momentum=0.9, weight_decay=0.00001)
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=3)
    schedulerPlugin = lr_scheduling.LRSchedulerPlugin(scheduler=scheduler)
    classBuffer = ClassBalancedBuffer(max_size=350)
    replayPlugin = ReplayPlugin(mem_size=350, storage_policy=classBuffer)
    masPlugin = MASPlugin(0.6)
